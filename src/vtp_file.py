    ...
    def get_packed_data(self, section):
        ...
        if section == 3:
            data_buffer += struct.pack(
                "<BBB", self.unknown1, len(self.constant_list), self.unknown2)
        ...
        for i in range(0, self.num_items):
            ...
            if num_types != 0:
                for i in range(0, num_types):
                    ...
                    unknown0, number_of_objects = struct.unpack(
            ...
            if self.section == 3:
                ...
                for i in range(0, num_constants):
                    unknown3, length = struct.unpack(
                        "<BI", file_pointer.read(5))
                    material_name, = struct.unpack(
                        "%ss" % length, file_pointer.read(length))
