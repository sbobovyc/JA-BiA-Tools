...
class CUI_ui_element_vertex:
    ...
    def get_packed_data(self):
        data_buffer = struct.pack(
            "<IIIB", self.vertex_id, self.color, *self.data)
        return data_buffer
...
#
    def __str__(self):
        ...
        # self.resource_id, self.ulx, self.uly, self.lrx, self.lry)
...
class CUI_ui_resource:
    ...
    def get_packed_data(self):
        data_buffer = struct.pack(
            "<II%isI%is" % (len(self.ui_name), len(self.filename)),
...
class CUI_data:
    ...
    def unpack(self, file_pointer, peek=False, verbose=False):
        self.last_variable_id, unknown1 = struct.unpack(
            "<II", file_pointer.read(8))
        ...
        for i in range(0, count):
            ...
            if binascii.hexlify(trailer) == magick1:
                ...
                if(trailer_length == 0):
                    ...
                    trailer_type = CUI_ui_element_trailer(
                        unknown_data0, unknown_data1)
...
#                print hex(region), x_offset, y_offset
        ...
        # 7D 7D 7D 64 (RGBA) diffuse (D3DDECLTYPE_D3DCOLOR) color, 7F 7F 7F C8
        # specular? (it's unused),

        # Pic_Background_white(solid), uint32 id, unint32 length, name, 19 bytes of stuff,
        # 7D 7D 7D 0A (RGBA) diffuse color, 7F 7F 7F C8 specular? (it's
        # unused),
        ...
        # MAIN_LoadGame, uint32 UI type(0xA7), 0xFFFFFFFF, 0x03030000, uint32
        # length of name, name,
        ...
        # command (play tutorial, new game, exit, etc) {memory address?},
        # nonsense
