...
class CTX_ID:
    ...
    def __repr__(self):
        return "%s(name=%r, id=%r, id_name=%r, path=%r)" % (self.__class__.__name__, self.id, self.id_name, self.path)
...
#            print binascii.hexlify(data_buffer)
    ...
    def __repr__(self):
        return "%s(name=%r, language=%r, data=%r)" % (self.__class__.__name__, self.description_string, self.data_dictionary)
        ...
        with con:
            ...
            if self.data.num_languages > 0:
                ...
                for description in tables:
                    language = CTX_language(description[0].encode('ascii', 'ignore'))  # convert unicode to ascii
